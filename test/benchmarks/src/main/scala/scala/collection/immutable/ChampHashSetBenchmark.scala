...
class ChampHashSetBenchmark {
  ...
  var xs: ChampHashSet[String] = _
  var ys: ChampHashSet[String] = _
  var zs: ChampHashSet[String] = _
  var zipped: ChampHashSet[(String, String)] = _
  var randomIndices: scala.Array[Int] = _
  def fresh(n: Int) = ChampHashSet((1 to n).map(_.toString): _*)

  var _1000ids : Array[String] = scala.Array.tabulate(1000)(i => i.toString)
...
//
  ...
  def access_contains(bh: Blackhole): Unit = {
    var i = 0
    while (i < 1000) {
      bh.consume(xs.contains(_1000ids(i)))
      i += 1
    }
  }
