...
class LinkerToolCommandLine(args: Array[String]){
  ...
  def classPath = scala.Option(classPathV).getOrElse(inputFile.getCanonicalPath)

  @ArgsjOption(name="-cpJ", aliases=Array("--usJavaClassPath"),
    usage="use java classpath"  )
  private var javaCP:Boolean = _
  def useJavaClassPath = javaCP
...
//
  ...
  def init(args:Array[String]): Unit = {
    ...
    require(overwrite || !outputFile.exists, s"output $outputFile exists - full path ${outputFile.getCanonicalPath}")
