  ...
  class HyperlinkAction(override val compiler: Global)
    ...
    override def runTest() {
      askAllSources(HyperlinkMarker) { pos =>
        ...
        if(symbol == compiler.NoSymbol || symbol == null) {
          reporter.println("\nNo symbol is associated with tree: "+tree)
        }
        else {
          reporter.println("\naskHyperlinkPos for `" + symbol.name + "` at " + format(pos) + " " + pos.source.file.name)
          ...
          val treePath = if (symbol.sourceFile ne null) symbol.sourceFile.path else null
          val treeName = if (symbol.sourceFile ne null) symbol.sourceFile.name else null

          sourceFiles.find(_.path == treePath) match {
            case Some(source) =>
              compiler.askLinkPos(symbol, source, r)
              r.get match {
                case Left(pos) =>
                  val resolvedPos = if (symbol.pos.isDefined) symbol.pos else pos
                  withResponseDelimiter {
                    reporter.println("[response] found askHyperlinkPos for `" + symbol.name + "` at " + format(resolvedPos) + " " + symbol.sourceFile.name)
                  }
                case Right(ex) =>
                  ex.printStackTrace()
              }
